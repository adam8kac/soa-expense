...
from fastapi import APIRouter, Path, status, Query, HTTPException, Body
...
async def create_expense(expense: ExpenseRequest, user_id: str = Path(...)):
...
async def get_expense(
    user_id: str = Path(...), date_from: date = Query(None), date_to: date = Query(None)
):
...
@router.get("/reports")
async def get_all_report_ids(user_id: str = Path(...)):
    try:
...
@router.get("/report")
async def get_report_by_id(user_id: str = Path(...), report_id: str = Query(...)):
    try:
...
async def delete_report_by_id(user_id: str = Path(...), report_id: str = Path(...)):
    try:
...
async def delete_all_reports(user_id: str = Path(...)):
    try:
...
async def delete_all_expenses(user_id: str = Path(...)):
    try:
...
async def delete_expense_by_id(user_id: str = Path(...), expense_id: str = Path(...)):
    try:
